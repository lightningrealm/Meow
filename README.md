...
| **导航路由** | Navigation Compose 3 | 多返回栈架构设计，提供类型安全（Type-Safety）的参数传递 |
| **架构模式** | MVI + UDF | 统一状态管理，状态可预测、行为可追溯，彻底消除状态撕裂 |
| **异步处理** | Kotlin Coroutines + Flow | 响应式异步数据流管理，轻松驾驭高频及周期性状态同步 |
| **网络请求** | Retrofit + OkHttp | RESTful API 健壮调用，集成高效拦截器与缓存控制 |
| **依赖注入** | Koin | 超轻量级运行期 DI 框架，无编译期代码生成暗坑，完美契合 Compose |
| **图片加载** | Coil 3 | 专为 Compose 深度优化的现代化异步图片加载库 |
| **音频播放** | Media3 (ExoPlayer) | 现代 Android 媒体库，提供坚固的后台播放能力与 MediaSession 控制 |
| **视觉特效** | AGSL (Android Graphics Shading Language) | 独家高性能着色器渲染，实现硬件加速级视觉动效 |
| **工程结构** | Gradle 模块化 | 严格按业务与核心能力解耦的松耦合多模块（Multi-Module）架构 |
...
```text
Meow/
├── app/                          # 主应用模块 (Application 壳工程，负责全局配置与初始化)
├── core/                         # 核心基础模块 (Core Modules，高复用低耦合)
│   ├── common/                   # 通用工具类、扩展函数及基础定义
│   ├── network/                  # 网络层封装，包含 Retrofit 实例与网易云 API 映射
│   ├── database/                 # 本地 Room 数据库存储与缓存控制
│   └── ui/                       # 全局通用 UI 组件 (如自定义 Slider、毛玻璃组件)
├── feature/                      # 业务功能模块 (Feature Modules，按业务边界拆分)
│   ├── login/                    # 登录功能模块
│   ├── home/                     # 首页内容、每日推荐、推荐歌单
│   ├── search/                   # 全局搜索与结果展现
│   ├── player/                   # 核心播放器详情页、动态歌词滚动
│   └── profile/                  # 个人中心、用户资产管理
...
* **`./gradlew`** (*Gradle Wrapper*)：使用项目内置的 Gradle 包装器脚本。它能确保团队内所有开发者使用完全一致的 Gradle 版本，避免由于本地环境差异引发构建失败。
